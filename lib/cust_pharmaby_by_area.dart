...
class CustPharmacyByAreaScreen extends StatefulWidget {
  @override
  _CustPharmacyByAreaScreenState createState() => _CustPharmacyByAreaScreenState();
}
  ...
  void getNearestShops() async {
    ...
    if (query.docs.isNotEmpty) {
      for (var i = 0; i < query.docs.length; i++) {
        var shopData = await FirebaseFirestore.instance.collection('Retailer').doc(query.docs[i].id).collection('Shopdata').get();
        if (shopData.docs.isNotEmpty) {
          if (pincodecontroller.text.isEmpty) {
            ...
            shopDatas.contactNUmber = shopData.docs[0].data()['contact'].toString();
                      ...
                      itemBuilder: (context, index) {
                        return GestureDetector(
                          ...
                          child: Card(
                            elevation: 0.8,
                            child: ListTile(
                              title: Text("Shop Name : " + listShopID[index].shopname!),
                              subtitle: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  Text("Contact Number : " + listShopID[index].contactNUmber!),
